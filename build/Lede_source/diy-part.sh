...
cat >$NETIP <<-EOF
uci set network.lan.ipaddr='10.6.9.2'                      # IPv4 地址(openwrt后台地址)
uci set network.lan.netmask='255.255.254.0'                   # IPv4 子网掩码
uci set network.lan.gateway='10.6.9.1'                    # 旁路由设置 IPv4 网关（去掉uci前面的#生效）
...
uci set system.@system[0].hostname='OpenWrt-Angel'              # 修改主机名称为OpenWrt-123
...
sed -i "s/OpenWrt /OpenWrt_x64_旁路由版 by Angel build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" "${ZZZ_PATH}"
